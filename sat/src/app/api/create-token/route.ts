...
const Body = z.object({
  ...
  maxSupply: z.string().regex(/^[0-9]+n?$/),  // "0" for unlimited
  freezable: z.coerce.boolean().default(true),
});

export async function POST(req: Request) {
  const ip = req.headers.get('x-forwarded-for') || 'local';
  const gate = rateLimit('create:' + ip, 6, 30_000);
  ...
  try {
    ...
    const maxSupplyBig = BigInt(b.maxSupply.endsWith('n') ? b.maxSupply.slice(0, -1) : b.maxSupply);

...
/** Normalisasi mode verifier dari ENV */
...
/** Variasi tampilan angka untuk teks scraping */
...
/** === Verifikasi TX via SparkScan API lalu fallback ke scraping.
 * Lulus jika:
 * - to.identifier === feeAddress
 * - (opsional) from.identifier === payer
 * - (opsional) amountSats >= minAmount
 * - status ∈ confirmed/completed/success
 * Return juga amountSats (kalau tersedia) untuk mode PROPORTIONAL.
 */
...
/** Verifikasi di halaman address (fallback/non-TX):
 * Cari keberadaan nominal unik + (opsional) payer.
 */
...
/** (Tambahan) export kumpulan untuk debug/import alternatif */

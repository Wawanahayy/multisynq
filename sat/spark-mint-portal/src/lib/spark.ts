...
/** Ambil alamat Spark issuer (kompatibel lintas versi). */
...
/** TokenTransactionService dari SDK (punya fallback berbagai konstruktor). */
...
/** ==== START helpers untuk create/mint & broadcast ==== */
...
async function signFinalizeBroadcast(tts:any, tx:any){
  ...
  // coba tarik txId dari SDK helper kalau ada
  const sdk = getSdk();
  try{
...
/** Public helpers dipakai oleh route dev/admin */
...
export async function computeTokenIdentifier(ticker:string, issuerSparkAddress:string){
  const sdk = getSdk();
  // pakai network dari address jika helper ada
...
/** Dump permukaan API untuk debug dev route */

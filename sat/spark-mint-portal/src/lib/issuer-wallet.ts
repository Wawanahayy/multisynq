...
async function getIssuerWalletViaIssuerSdk(){
  ...
  // IssuerSparkWallet.initialize({ mnemonicOrSeed, options:{network} })
...
async function newTokenServiceViaSparkSdk(wallet:any){
  ...
  if (!TT) return null; // biar langsung fallback
  ...
  if (!tts){
    // Pseudo instance: set properti agar method internal tidak baca undefined
...
async function newTokenServiceViaIssuerSdk(wallet:any){
  ...
  // cari TokenTransactionService di issuer-sdk juga
  const TT =
...
/* =========================
 *  PUBLIC: buat TTS (pilih flavor)
 * ========================= */
export async function newTokenService(wallet:any){
  // Prioritas bisa dipaksa via ENV
  const prefer = String(process.env.MINT_SDK_FLAVOR || '').toUpperCase(); // 'SPARK'|'ISSUER'|''
  if (prefer === 'ISSUER') {
    return (await newTokenServiceViaIssuerSdk(
      wallet ?? await getIssuerWalletViaIssuerSdk()
    )) || (await newTokenServiceViaSparkSdk(wallet));
  }
  // default: coba spark-sdk dulu, lalu issuer-sdk
...
async function startMintTokenTx(tts:any, p:{tokenIdentifier:string;tokenAmount:bigint;toSparkAddress:string}){
  ...
  // coba fungsi global di SDK yang terkait
...
export async function mintAndBroadcast(tts:any, p:{tokenIdentifier:string;tokenAmount:bigint;toSparkAddress:string}){
  // 1) coba langsung dengan TTS yang diberikan
  let started = await startMintTokenTx(tts, p);
  if (!started.ok) {
    // 2) fallback: bangun TTS lain (spark-sdk/issuer-sdk) dan coba lagi
    try {
      const altTts =

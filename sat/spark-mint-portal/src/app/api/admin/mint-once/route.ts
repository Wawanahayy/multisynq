...
const Body = z.object({
  tokenIdentifier: z.string().optional(),     // fallback ke env PAYMINT_TOKEN_IDENTIFIER
  amount: z.string().regex(/^[0-9]+$/).optional(), // base units; default "1"
  receiver: z.string().optional(),            // fallback ke env ISSUER_SPARK_ADDRESS atau address issuer dari wallet
});
...
export async function POST(req: Request){
  ...
  try{
    ...
    // receiver: body → env → issuer sendiri
    let receiver = (b.receiver || process.env.ISSUER_SPARK_ADDRESS || "").trim();
    if (!receiver) {
      // derive dari wallet jika env kosong
      // @ts-ignore - asumsi wallet punya method ini
    ...
    const amountStr = b.amount || "1"; // 1 base unit aja biar cepat kedetect
    const amount = BigInt(amountStr);

    // panggil SDK mint (nama method bisa berbeda tergantung versi SDK)
    // Coba variasi umum:

...
// read bigint (0 allowed) + presence flag
...
export async function GET() {
  ...
  // Fallback dari .env (prioritas PAYMINT_*, lalu NEW_TOKEN_*)
  ...
  // Fallback dari .env bila API tidak menyediakan maxSupply
  if (!maxPresent && /^[0-9]+$/.test(envMaxStr)) {
    maxPresent = true;
    maxVal = BigInt(envMaxStr);
  }

  // Kalau sama sekali tidak ada info minted & max
